...
const FOTODB_CLIENT = new pg.Client(FOTODB_CONNECTION_CONFIG)
...
async function QueryAccountInfo(username: string, password: string): Promise<boolean> {
    let query = await FOTODB_CLIENT.query(
...
async function SaveSession(username: string, sessionid: string): Promise<void> {
    await FOTODB_CLIENT.query(`INSERT INTO sessions (username, sessionid) VALUES ('${username}', '${sessionid}')`)
}


async function IsAlbumIdValid(albumid: string): Promise<boolean> {
    let query = await FOTODB_CLIENT.query(`SELECT EXISTS (SELECT albumid FROM albums WHERE albumid='${albumid}')`)
...
async function GetAlbums(username: string): Promise<AlbumEntries> {
    let query = await FOTODB_CLIENT.query(`SELECT album_name, albumid FROM albums WHERE username='${username}'`)
    return (query.rows as AlbumEntries)
}


async function GetAlbumNameByItsAlbumID(albumid: string): Promise<string> {
    let query = await FOTODB_CLIENT.query(`SELECT album_name FROM albums WHERE albumid='${albumid}'`)
